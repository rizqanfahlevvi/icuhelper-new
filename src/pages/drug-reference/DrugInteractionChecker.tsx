...
import { UnifiedInteraction, SMART_RULES, matchDrugWithInteractionTarget, checkPairInteraction } from './interactionUtils';
  ...
  const interactions = useMemo((): UnifiedInteraction[] => {
    ...
    for (let i = 0; i < selectedDrugs.length; i++) {
      for (let j = i + 1; j < selectedDrugs.length; j++) {
        ...
        const ix = checkPairInteraction(keyA, keyB);
        if (ix) {
          list.push(ix);
        }
      }
  ...
  return (
    <div 
      className={`p-4 rounded-2xl border transition-all ${
        isHighlighted 
          ? (isMajor ? 'ring-2 ring-red-500 border-red-500/40 bg-red-500/5' : 'ring-2 ring-amber-500 border-amber-500/40 bg-amber-500/5')
          : (isMajor ? 'bg-red-500/[0.02] dark:bg-red-500/[0.01] border-red-500/20 hover:border-red-500/30' : 'bg-amber-500/[0.02] dark:bg-amber-500/[0.01] border-amber-500/20 hover:border-amber-500/30')
      }`}
    >
